...
export default function RegisterPage() {
  ...
  if (success) {
    return (
      <div className="bg-background text-zinc-100 min-h-[calc(100vh-64px)] flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-card border border-zinc-800 p-12 text-center">
          <Icon icon="lucide:mail-check" className="mx-auto text-primary mb-6" width={48} />
          <h2 className="font-playfair text-2xl font-bold mb-4">Revisa tu correo</h2>
          <p className="text-zinc-400 text-sm font-light mb-8">
            Te hemos enviado un enlace a <strong>{email}</strong> para confirmar tu cuenta. Por favor haz clic en él para activar tu acceso a LYON CALL.
          </p>
          <Link href="/" className="text-sm uppercase tracking-widest border border-zinc-700 px-6 py-3 hover:border-primary hover:text-primary transition-colors">
            Volver al Inicio
          </Link>
        </div>
      </div>
      ...
      <div className="w-full max-w-md bg-card border border-zinc-800 p-8 sm:p-12 shadow-2xl">
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-muted border border-zinc-800 p-3 text-sm focus:border-primary focus:outline-none transition-colors text-white" 
              />
            </div>
            <div>
          ...
          <button 
            type="submit"
            disabled={isLoading}
            className="w-full bg-white text-black py-4 uppercase tracking-widest text-sm font-bold flex items-center justify-center gap-2 hover:bg-primary hover:text-white transition-colors mt-6 disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="mt-8 pt-6 border-t border-zinc-800 text-center text-sm text-zinc-400 font-light">
          ...
          <Link href="/auth/login" className="text-white hover:text-primary font-medium transition-colors">
            Inicia sesión
          </Link>
        </div>
      </div>
