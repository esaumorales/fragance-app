        ...
        <div className="bg-[#141414] border border-zinc-800 p-8">
          {(!orders || orders.length === 0) ? (
            <div className="text-center py-12">
              ...
              <Link href="/perfumes" className="bg-white text-black px-8 py-3 text-xs font-bold uppercase tracking-widest hover:bg-[#d4af37] hover:text-white transition-colors">
                Explorar Catálogo
              </Link>
            </div>
          ) : (
            <div className="space-y-6">
              {orders.map((order) => (
                <div key={order.id} className="border border-zinc-800 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-[#d4af37]/50 transition-colors">
                  <div>
                  ...
                  <div className="flex flex-col md:items-end gap-2">
                    ...
                    <span className="bg-[#1a1a1a] px-3 py-1 text-xs uppercase tracking-widest text-zinc-400">
                      {order.status}
                    </span>
                  </div>
                </div>
